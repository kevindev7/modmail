...
client.on('ready', () => {
    console.log(`Logged in as ${client.user.username} and Bot is Ready to use on ${client.guilds.cache.size} servers.`);
    client.user.setActivity(`Help, DM for Support`, { type: "WATCHING"})
});

require("http").createServer((_, res) => res.end(`The Bot Is Ready to use on K V N Community servers!`)).listen(8080)
...
client.on("message", async message => {
    ...
    if (message.guild) {
        ...
        if (command == "setup") {
            ...
            await message.guild.channels.create("K MODMAIL", {
                type: "category",
...
//Close Modmail
            ...
            if (!message.content.startsWith(prefix)) return;
            if (!message.member.roles.cache.find((x) => x.name == "💻・Λdmin")) {
                return message.channel.send("You need 💻・Λdmin role to use this command")
            }
            if (message.channel.parentID == message.guild.channels.cache.find((x) => x.name == "K MODMAIL").id) {
            ...
            if (!message.content.startsWith(prefix)) return;
            const category = message.guild.channels.cache.find((x) => x.name == "K MODMAIL")
...
//Modmail Engine
    ...
    if (!message.guild) {
        ...
        if (!guild) return;
        const category = guild.channels.cache.find((x) => x.name == "K MODMAIL")
        if (!category) return;
